...
function makeOpportunity(overrides: Partial<ResearchOpportunity> = {}): ResearchOpportunity {
  ...
  return {
    ...
    decisionReasons: ["Fixture AUTO_ELIGIBLE opportunity"],
    failedGates: [],
...
test("AUTO_ELIGIBLE scheduled cycle creates a complete article preserving the brief", async () => {
  const db = createDb(databaseUrl);
  await migrate(db);
  ...
  const opportunity = makeOpportunity({ decision: "AUTO_ELIGIBLE" });
  const slot = `research:auto-pipeline:${Date.now()}`;
  let publishCalls = 0;
  ...
  const result = await executeScheduledResearchCycle({
    ...
    deps: {
      ...
      publishArticle: async () => {
        publishCalls += 1;
        throw new Error("draft_only mode must not publish");
      },
...
test("DRAFT_ONLY opportunity is not auto-generated by the scheduler", async () => {
  const db = createDb(databaseUrl);
  await migrate(db);
  await withSettings(db, {
    ...defaultSettings,
  ...
  const opportunity = makeOpportunity({
    decision: "DRAFT_ONLY",
    decisionReasons: ["Fixture DRAFT_ONLY — must not auto-generate"]
  });
  let generated = 0;
  const result = await executeScheduledResearchCycle({
    db,
    config: fakeConfig,
    slotKey: `research:draft-refused:${Date.now()}`,
    actor: "test",
    deps: {
      loadContentInventory: async () => emptyInventory(),
      runResearchCycle: async () => makeCycleResult(opportunity),
  ...
  assert.ok(result.reasons.some(r => /AUTO_ELIGIBLE|DRAFT_ONLY|filler/i.test(r)));
  await db.end();
});
