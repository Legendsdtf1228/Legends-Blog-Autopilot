...
import { createDb, migrate, createArticle, listArticles, getSettings } from "../src/db.js";
import {
...
export { LOCAL_SMALL_BUSINESS_STORIES_FIXTURE };
...
test("quarantine archives the exact failed article and excludes it from rollout counting", async () => {
  ...
  // Real counter advancement is covered in refinement-rollout-regression.test.ts
  const settings = await getSettings(db);
  assert.equal(settings.draftOnlyMode, true);
  assert.notEqual(settings.rolloutMode, "auto_publish");
});
